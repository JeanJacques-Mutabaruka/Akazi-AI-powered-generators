...
def init_session_state():
    ...
    if 'duplicate_files' not in st.session_state:
        st.session_state.duplicate_files = []  # ✅ Noms des fichiers ignorés (doublons)
...
def reset_all():
    ...
    for key in ['files_data', 'duplicate_files', 'generated_documents', 'processing_logs',
...
if uploaded_files:
    ...
    if 'last_files_hash' not in st.session_state or st.session_state.last_files_hash != current_files_hash:
        st.session_state.files_data = []
        st.session_state.duplicate_files = []  # ✅ Reset à chaque nouveau batch
        ...
        for idx, uploaded_file in enumerate(uploaded_files, 1):
            try:
                ...
                if content_hash in existing_content_hashes:
                    ...
                    # ✅ Stocker le doublon pour l'afficher dans la table
                    st.session_state.duplicate_files.append(uploaded_file.name)
                    continue
    ...
    with tab1:
        ...
        if st.session_state.files_data:
            ...
            # ── TABLEAU PRINCIPAL : fichiers actifs avec checkboxes interactives ──
            table_data = []
            ...
            # ✅ df simple (pas de Styler) → checkboxes interactives
            ...
            # ── TABLEAU DOUBLONS : lecture seule, séparé, fond rouge ──
            if st.session_state.duplicate_files:
                st.markdown("""
                <div style='margin-top: 20px; padding: 10px 16px;
                            background: #ffe0e0; border-left: 4px solid #cc0000;
                            border-radius: 6px;'>
                    <strong style='color: #cc0000;'>
                        🔴 Fichiers ignorés — contenu identique déjà chargé
                    </strong>
                </div>
                """, unsafe_allow_html=True)

                dup_rows = [{'Fichier ignoré': name, 'Raison': '🔴 Contenu identique à un fichier déjà chargé'}
                            for name in st.session_state.duplicate_files]
                dup_df = pd.DataFrame(dup_rows)

                styled_dup = dup_df.style.set_properties(**{
